  ...
  public async findUserByCode(code: string): Promise<TypeUserInfo> {
    ...
    if (!tokenRequest.ok) {
      throw new BadRequestException(
        `Не вдалось отримати користувача з ${this.options.profile_url}`,
      );
    }
    const tokens = await tokenRequest.json();
    if (!tokens.access_token) {
      throw new BadRequestException(
        `Немає токенів авторизації з ${this.options.access_url}`,
      );
    }
    ...
    if (!userRequest.ok) {
      throw new UnauthorizedException(`Не вдалось отримати користувача`);
    }
