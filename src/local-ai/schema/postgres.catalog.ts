...
/**
 * Каталог таблиць Postgres, які має право читати локальна модель.
 *
 * ЦЕ Й Є ВИДАЧА ДОСТУПУ: SqlGuardService будує whitelist саме з цього файлу,
 * тому таблиця, якої тут немає, для моделі не існує — запит до неї буде відхилено
 * ще до звернення до БД. Таблиці з логінами (`usr`), сесіями й системні каталоги
 * сюди не додавати.
 *
 * Колонки нижче — резервний список: у робочому режимі SchemaCatalogService
 * перечитує їх із information_schema разом із коментарями (`COMMENT ON COLUMN`),
 * тож реальна база завжди має пріоритет над цим файлом. Тут лишається те, чого в
 * метаданих БД немає: призначення таблиці, значення статусів і правила рахунку.
 *
 * Oracle у каталозі свідомо відсутній — для локальної моделі він вимкнений.
 */
